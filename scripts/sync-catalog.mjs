...
async function fetchText(url, label) {
  const response = await fetch(url, {
    headers: { 'User-Agent': 'CatchGrid catalog sync/3' },
  });
  if (!response.ok) throw new Error(`${label}: HTTP ${response.status}`);
  const raw = await response.text();
...
function historicalShadowKeys(html) {
  const tableStart = html.indexOf('id="go-shadow"');
  const tableEnd = html.indexOf('</table>', tableStart);
  if (tableStart < 0 || tableEnd < 0) throw new Error('Historical Shadow table was not found.');
  const keys = new Set();
  for (const match of html.slice(tableStart, tableEnd).matchAll(/<tr>[\s\S]*?<\/tr>/g)) {
    const row = match[0];
    const dex = Number(row.match(/data-sort-value="(\d+)"/)?.[1]);
    if (!dex) continue;
    const regional = row.match(
      /<small class="text-muted">(Alolan|Galarian|Hisuian|Paldean)\b/i,
    )?.[1];
    const origin = regional
      ? { alolan: 'alola', galarian: 'galar', hisuian: 'hisui', paldean: 'paldea' }[
          regional.toLowerCase()
        ]
      : null;
    keys.add(`${dex}:${origin ? `regional-${origin}` : 'standard'}`);
  }
  return keys;
}
...
function rulesFor(form) {
  if (!form.isDefault) {
    return {
      ...
      xxs: 'ineligible',
      shadow:
        form.release.shadow === true
          ? 'released'
          : form.release.shadow === false
            ? 'ineligible'
            : 'ineligible',
      purified:
        form.release.purified === true
          ? 'released'
          : form.release.purified === false
            ? 'ineligible'
            : 'ineligible',
    };
  }
...
function makeSourceInputs(
  ...
  inputs.push({
    key: 'historical-shadow-pokemon-db',
    kind: 'secondary',
    url: overrides.historicalShadowSource.url,
    sha256: historicalShadow.sha256,
    effectiveAt: overrides.releaseMetadataAsOf,
    note: 'Hash covers the complete historical Shadow table used for form-aware eligibility.',
  });
  inputs.push({
...
function parseBaselineRules(sql) {
  return new Map(
    [
      ...sql.matchAll(
        /\('([^']+)', '(normal|shiny|lucky|hundo|xxl|xxs|shadow|purified)', '([^']+)'/g,
      ),
    ].map((match) => [`${match[1]}:${match[2]}`, match[3]]),
  );
}

function buildReport(manifest, overrides, baselineRules) {
...
async function main() {
  ...
  const baselineRules = parseBaselineRules(
    await readFile(resolve(ROOT, 'migrations/0009_phase_b_catalog_data.sql'), 'utf8'),
  );
  const [hashResponse, assetTree, historicalShadow, ...feedResponses] = await Promise.all([
    fetchJson(`${API_BASE}/api_hashes.json`, 'PoGoAPI hashes'),
    getAssetTree(overrides.assetCommit),
    fetchText(overrides.historicalShadowSource.url, 'historical Shadow catalog'),
  ...
  if (historicalShadow.sha256 !== overrides.historicalShadowSource.sha256) {
    throw new Error(
      `Historical Shadow source changed (expected ${overrides.historicalShadowSource.sha256}, received ${historicalShadow.sha256}). Review the table before updating the pinned hash.`,
    );
  }
  const shadowKeys = historicalShadowKeys(historicalShadow.raw);
  for (const input of API_INPUTS) {
    const filename = `${input}.json`;
  ...
  for (let dex = 1; dex <= overrides.nationalDexMax; dex += 1) {
    ...
    const isShadow =
      isReleased &&
      (shadowKeys.has(`${dex}:standard`) || Boolean(shadow[dex] ?? shadow[String(dex)]));
    ...
    const form = {
      ...
      sourceIds: [
        ...
        ...(isShadow ? ['historical-shadow-database'] : []),
  ...
  for (const group of overrides.formGroups) {
    for (const [memberIndex, member] of group.members.entries()) {
      ...
      const historicalShadowKey = group.regionalOrigin
        ? `${member.dex}:regional-${group.regionalOrigin.toLowerCase()}`
        : null;
      const isHistoricalShadow = Boolean(
        historicalShadowKey && shadowKeys.has(historicalShadowKey),
      );
      const form = {
        ...
        release: {
          ...
          shadow: isHistoricalShadow,
          purified: isHistoricalShadow,
        },
        ...
        sourceIds: [
          ...(member.sourceIds ?? group.sourceIds),
          ...(isHistoricalShadow ? ['historical-shadow-database'] : []),
        ],
  ...
  const report = buildReport(manifest, overrides, baselineRules);
